...
import FARM_CONFIGS, { FOX_USDC_PID } from "@/config/data/farm-configs";
import { getAddress, getMasterChefAddress } from "@/hooks/web3/address-helpers";
import multicall, { Call } from "@/hooks/web3/multicall";
import {
  BIG_ONE,
  BIG_ZERO,
  BLOCKS_PER_YEAR,
...
import TOKENS, { Token, TokenSymbol } from "@/config/data/tokens";
...
const getPrice = (prices: LpPrices, token: TokenSymbol): BigNumber => {
  if (
    ...
    token === TokenSymbol.UST ||
    token === TokenSymbol.USDT ||
    token === TokenSymbol.BSCBUSD ||
    token === TokenSymbol.BUSD ||
...
export const getFarms = async (lpPrices: LpPrices): Promise<FarmsResults> => {
  ...
  const farms = FARM_CONFIGS.map((farmConfig, i) => {
    ...
    if (farmConfig.pid === 0) {
      ...
      tokenPriceVsQuote = tokenAmount.isZero()
        ? BIG_ONE
