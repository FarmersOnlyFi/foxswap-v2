import { getFactoryAddress } from "@/hooks/web3/address-helpers";
...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "fox/subgraph") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const POSITIONS_BY_BLOCK = (account: string, blocks: any) => {
  let queryString = "query blocks {";
...
export const SHARE_VALUE = (pairAddress: string, blocks: any) => {
  let queryString = "query blocks {";
  queryString += blocks.map(
    (block: any) => `
      t${block.timestamp}:pair(id:"${pairAddress}", block: { number: ${block.number} }) { 
...
export const USER = (block: any, account: string) => {
  const queryString = `
    query users {
...
export const PAIRS_HISTORICAL_BULK = (block: any, pairs: Pair[]) => {
  let pairsString = `[`;
  pairs.map((pair: Pair) => {
    return (pairsString += `"${pair}"`);
  });
...
`;
export const TOKENS_HISTORICAL_BULK = (tokens: any, block: any) => {
  let tokenString = `[`;
